...
class HockeyPlayer:
    ...
    def stuck(self, kart_loc):
        threshold = 2
        for last_loc in reversed(self.past_locs):
    ...
    def act(self, image, player_info):
        ...
        image_puck_loc = self.model(image_transform).squeeze(0).detach().cpu().numpy()
        # print(image_puck_loc)
        ...
        if self.state == 'kickoff':
            ...
            action = self.attack_action(kart_loc, kart_front, puck_loc, action)
        elif self.state == 'positioning':
            action = self.positioning_action(kart_loc, kart_front, puck_loc, action)
        ...
        # print('past_locs:', self.past_locs, '\npast_actions:', self.past_actions)
